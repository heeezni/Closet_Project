...
function saveClothesForm(){
    ...
    addForm.addEventListener("submit", (e) => {
        ...
        reader.onload = function(event){ //위에 e있음
            let imageData = event.target.result; //base64 문자열
        ...
        reader.readAsDataURL(file); //base64변환 시작
    });
}
    ...
    request.onsuccess = function(){
        ...
        if(cursor){//cursor 
            ...
            if (categoryMatch && seasonMatch && colorMatch && patternMatch){
                ...
                <img src="${item.image}" alt="${item.name || item.clothesname}" class="clothing-img">
                <h3>${item.name}</h3>
...
function renderForCoordi(){
    ...
    request.onsuccess = function(){
        ...
        for(let i=0; i<closet.length; i++){
            ...
            card.innerHTML = `<img src="${item.image}" alt="${item.name || item.clothesname}" class="clothing-img">
            <p>${item.name}</p>`;
...
function resetAddForm() {
    document.getElementById("addForm").reset(); // 기본값 초기화
